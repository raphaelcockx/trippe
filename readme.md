...
Unofficial API client to check hotel prices and reward nights availability at over 6.000 hotels worldwide - suitable for an intercontinental lifestyle. Please ask for permission of the hotel company before using this software in production. Please also note that Trippe will **not** work 'out of the box' but needs a (static) API key to perform any request to the backend.

[![npm version](https://img.shields.io/npm/v/trippe.svg)](https://www.npmjs.com/package/trippe)
[![license](https://img.shields.io/github/license/raphaelcockx/trippe.svg?style=flat)](LICENSE)

## First of all

This is both a labour of love and a work in progress. During development Trippe has been tested and fine-tuned thoroughly, things like method names and the structure of both parameters as well as output is unlikely to change anytime soon. There are however a sheer infinite number of combinations when it comes to hotel, prices and dates meaning 'special cases' are bound to keep popping up. If you find a bug or just notice that the output of a certain search makes no sense at all, make sure to [open an issue](https://github.com/raphaelcockx/trippe/issues).
...
The `getHotelDetails` method will return some basic info on a specific hotel in the system. It need a single parameter, in the form of the unique `hotelCode` of the property. This id of 5 characters (all letters, all uppercase) will be used for almost every other method as well.
...
| closestCity | String | The closest city to the hotel (may be the actual city of the address)
...
| country | String | The country the hotel is in, expressed as a ISO 3166-1 alpha-2 code
...
```js
// Get all available rates for a 2 night stay in Tokyo
...
Although queries are obviously somewhat language specific, `getDestinations` can for the moment only work with query strings in English. More language support is coming soon.
...
```js
// Get lowest prices for a 1 night stay in a radius of 20 km around the Place de la Concorde in Paris
...
**Please note** that this method will not include hotels that have no rooms available. You should therefor not use this method to get a list of hotels in a certain area.

---
### `getLowestHotelPrices(hotelCode, [options])`

The `getLowestHotelPrices` method will do more or less the opposite of the `getLowestAreaPrices` method above. It will provide you with pricing data from one single hotel (as specified by `hotelCode`) but spread out over a period of up to 60 days. This 'price calender' shows both cash rates as well as prices in points. It is important to note though that cash prices **DO NOT** include taxes - see below for more details.
...
```js
// Book a 2 night stay in early March in Tokyo
